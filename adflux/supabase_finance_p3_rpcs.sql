...
--
-- finance_pnl_summary(from,to,segment) → full P&L dashboard bundle for the mockup:
--   income (CRM approved payments), costs (bank ledger direct+common), operating
--   profit/margin, by_segment net, per_company, revenue_mix (segment+media),
--   monthly trend, expense by_head, cash-movements (loans/transfers/drawings/tax),
--   assets/investments list, review flag.
-- finance_accounts_home() → accountant workspace: to-collect (receivables),
--   approvals pending, review count, finance_tasks (today/overdue/upcoming).
-- Both SECURITY DEFINER + role gate: admin/accounts full; co_owner+government_partner
  ...
  v_out := jsonb_build_object(
    ...
    'revenue_mix', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
               'segment', segment, 'media', COALESCE(media_type,'—'),
               'label', (CASE WHEN segment='GOVERNMENT' THEN 'Govt' ELSE 'Pvt' END)||' · '||COALESCE(media_type,'Other'),
               'amount', amt, 'pct', CASE WHEN itot>0 THEN round(amt/itot*100) ELSE 0 END) ORDER BY amt DESC)
      FROM (SELECT q.segment, q.media_type, SUM(p.amount_received) amt
              FROM public.payments p JOIN public.quotes q ON q.id=p.quote_id
             WHERE p.approval_status='approved'
               AND (p_from IS NULL OR p.payment_date>=p_from) AND (p_to IS NULL OR p.payment_date<=p_to)
               AND (v_seg IS NULL OR q.segment=v_seg)
             GROUP BY q.segment, q.media_type) m), '[]'::jsonb),
    'monthly', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('month', mth, 'income', COALESCE(inc,0), 'cost', COALESCE(cst,0)) ORDER BY mth)
      FROM (
        SELECT mth, MAX(inc) inc, MAX(cst) cst FROM (
          SELECT to_char(p.payment_date,'YYYY-MM') mth, SUM(p.amount_received) inc, NULL::numeric cst
            FROM public.payments p JOIN public.quotes q ON q.id=p.quote_id
           WHERE p.approval_status='approved'
             AND (p_from IS NULL OR p.payment_date>=p_from) AND (p_to IS NULL OR p.payment_date<=p_to)
             AND (v_seg IS NULL OR q.segment=v_seg)
           GROUP BY 1
          UNION ALL
          SELECT to_char(txn_date,'YYYY-MM') mth, NULL, SUM(amount)
            FROM public.finance_transactions
    ...
    'assets', COALESCE((
      ...
      FROM public.finance_transactions
       WHERE bucket IN ('investment','asset')
         AND (p_from IS NULL OR txn_date>=p_from) AND (p_to IS NULL OR txn_date<=p_to)
         AND (v_seg IS NULL OR segment=v_seg OR segment IS NULL)), '[]'::jsonb),
...
BEGIN
  ...
  v_out := jsonb_build_object(
    -- receivables: won/sent quotes minus approved payments, per client, oldest first
    'to_collect', COALESCE((
      ...
      FROM (
        SELECT q.id, COALESCE(q.client_company, q.client_name, 'Client') client_name,
         ...
         WHERE q.status IN ('sent','negotiating','won')
           AND (NOT v_gov_only OR q.segment='GOVERNMENT')
