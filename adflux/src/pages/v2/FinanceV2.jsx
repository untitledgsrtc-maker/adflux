...
function PnlTab() {
  ...
  useEffect(() => {
    ...
    supabase.rpc('finance_pnl_summary', { p_from: null, p_to: null, p_segment: null })
      .then(({ data, error }) => { if (!alive) return; if (error) setErr(error.message); else setD(data || {}); setLoading(false) })
    return () => { alive = false }
  }, [])
  if (loading) return <Spin />
  if (err) return <Card style={{ borderColor: 'var(--danger)' }}><span style={{ color: 'var(--danger)' }}>Could not load: {err}</span></Card>
  if (!d || d.operating_profit == null) return <Card>No finance data yet - run the Phase 1-3 SQL.</Card>

  const profit = +d.operating_profit || 0, income = +d.income || 0
  const cost = (+d.direct_cost || 0) + (+d.common_expense || 0)
  const excl = d.excluded || {}, byTag = d.by_tag || [], monthly = d.monthly || [], review = d.review || { count: 0, amount: 0 }
  const moves = [
    ['Transfers between your own accounts', +excl.internal_transfer || 0, 'just moving money, not spending'],
    ['Loans (borrowed minus repaid)', (+excl.loan_in || 0) - (+excl.loan_out || 0), 'financing, not profit'],
    ['Your personal / drawings', +excl.owner_drawings || 0, 'money you took out'],
    ['Tax paid', +excl.tax || 0, 'government dues'],
  ]
  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
      <div style={{ position: 'relative', overflow: 'hidden', border: '1px solid var(--border-strong, #475569)', borderRadius: 20, padding: '24px 26px',
        ...
        <div style={{ fontSize: 11, letterSpacing: '.16em', textTransform: 'uppercase', color: 'var(--text-subtle)', fontWeight: 600 }}>Profit (Apr-Jul)</div>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Money in {fmtINR(income)} &middot; Money out {fmtINR(cost)} &middot; Margin {d.margin_pct}%</div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
        <Kpi label="Money In" value={fmtINR(income)} sub="received in the bank" Icon={TrendingUp} color="var(--success, #10B981)" tint="var(--success-soft, rgba(16,185,129,.12))" />
        <Kpi label="Money Out" value={fmtINR(cost)} sub="business + office spending" Icon={TrendingDown} color="var(--danger)" tint="var(--danger-soft, rgba(239,68,68,.12))" />
        <Kpi label="Profit" value={fmtINR(profit)} sub={`margin ${d.margin_pct}%`} Icon={BarChart3} color={profit >= 0 ? 'var(--success, #10B981)' : 'var(--danger)'} tint="var(--success-soft, rgba(16,185,129,.12))" />
      </div>

      <Card>
        <CH>Month by month <span style={{ color: 'var(--text-subtle)', fontWeight: 400, fontSize: 12 }}>money in vs out</span></CH>
        <div style={{ display: 'flex', gap: 14, marginBottom: 6, fontSize: 11, color: 'var(--text-muted)' }}>
          <span style={{ display: 'flex', gap: 6, alignItems: 'center' }}><Dot c="var(--blue, #3B82F6)" />In</span>
          <span style={{ display: 'flex', gap: 6, alignItems: 'center' }}><Dot c="var(--danger, #EF4444)" />Out</span>
        </div>
        <TrendChart data={monthly} />
      </Card>

      <Card style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '16px 18px 6px' }}><CH>By your categories <span style={{ color: 'var(--text-subtle)', fontWeight: 400, fontSize: 12 }}>straight from your Excel</span></CH></div>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead><tr><th style={{ ...thL, paddingLeft: 18 }}>Category</th><th style={thR}>Money In</th><th style={{ ...thR, paddingRight: 18 }}>Money Out</th></tr></thead>
          <tbody>
            {byTag.map((t, i) => (
              <tr key={i}>
                <td style={{ ...tdL, paddingLeft: 18 }}>{t.tag} <span style={{ color: 'var(--text-subtle)', fontSize: 11 }}>&middot; {t.rows}</span></td>
                <td style={{ ...tdR, color: +t.inflow ? 'var(--success)' : 'var(--text-subtle)' }}>{+t.inflow ? '+' + fmtINR(t.inflow) : '\u2014'}</td>
                <td style={{ ...tdR, paddingRight: 18, color: +t.outflow ? 'var(--text)' : 'var(--text-subtle)' }}>{+t.outflow ? fmtINR(t.outflow) : '\u2014'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Card>
        <CH>Not counted as profit <span style={{ color: 'var(--text-subtle)', fontWeight: 400, fontSize: 12 }}>money movements, not business earning</span></CH>
        {moves.map(([l, v, note]) => (
          <div key={l} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 2px', borderBottom: '1px solid var(--border-soft, rgba(255,255,255,.06))' }}>
            <div><div style={{ fontSize: 13.5 }}>{l}</div><div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{note}</div></div>
            <div style={{ fontFamily: 'Space Grotesk', fontWeight: 600, fontSize: 15 }}>{fmtINR(v)}</div>
          </div>
        ))}
      </Card>
      ...
      {review.count > 0 && (
        <div style={{ display: 'flex', gap: 9, alignItems: 'flex-start', background: 'var(--danger-soft, rgba(239,68,68,.12))', border: '1px solid rgba(239,68,68,.3)', borderRadius: 12, padding: '11px 14px', fontSize: 13, color: 'var(--text-muted)' }}>
          ...
          <div><b style={{ color: 'var(--text)' }}>{review.count} entries to sort</b> - {fmtINR(review.amount)} not yet in a category. Open the Register and set their category.</div>
        </div>
      )}
    </div>
  )
